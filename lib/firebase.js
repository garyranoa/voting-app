...
export async function newRound(sessionId) {  
  ...
  const roundNumber = sessionContent.hasOwnProperty("rounds")
    ...
    : 1;
  const voters = sessionContent.voters
    ? Object.keys(sessionContent.voters)
    : [];
  ...
  const data = {
    ...
    voting_state: VOTING_STATES.RUNNING,
    options: sessionContent.defaultOptions,
    timeState: 0
  };
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return set(roundRef, data)
    .then(() =>
      set(child(roundRef, "votes"), votes).catch((error) =>
        console.log(error)
      )
    )
    .catch((error) => error);
...
export async function updateWord(sessionId, roundNumber) {
  const question = await getRandomQuestion();
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return update(roundRef, { question: question, seed: generateRandomSeed() }).catch(
    (error) => error
  );
}

  export async function updateRoundState(sessionId, roundNumber, state, options = []) {
    const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
    ...
    const questionRef = child(sessions, `/${sessionId}/${questions}/${question.id}`);
    const questionSnapshot = await get(questionRef);
    if (questionSnapshot.val() == null) {
      update(questionRef, question).catch((error) => error);
    }
    if (options.length > 0) {
      return update(roundRef, { state: state, votingOptions: options }).catch((error) => error);
    }
...
export async function updateRoundVotingState(sessionId, roundNumber, state) {
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return update(roundRef, { voting_state: state }).catch((error) => error);
}
...
export async function updateUserVote(sessionId, roundNumber, username, vote) {
  ...
  return update(child(roundRef, `votes/${username}`), {
    vote: vote.name,
    seed: generateRandomSeed(),expire: 0,
  })
  .then(() => incrementOptionVote(sessionId, roundNumber, vote) )
  .catch((error) => error);
}

export async function incrementOptionVote(sessionId, roundNumber, vote, delta = 1) {
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  const optionRef = child(roundRef, `options/${vote.id}`)

    const optionSnapshot = await get(optionRef);
    if (optionSnapshot.val() == null) {
      await update(optionRef, {total: delta}).catch((error) => error);
    }else {
      await update(child(roundRef, `options/${vote.id}`), {total: increment(delta)})
    }
  
}

export async function updateUserTimeExpire(sessionId, roundNumber, username, expire) {
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return update(child(roundRef, `votes/${username}`), {
    expire: expire,
  }).catch((error) => error);
...
export async function updateUserActionVote(sessionId, roundNumber, username, action) {
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return update(child(roundRef, `votes/${username}`), {
    expire: 0,
    action: action
...
export async function importQuestion(_questions) {
  ...
  return set(child(sessions, questions), childData)
        .catch((error) => console.log(`Importing Questions error: ${error}`) );
}
