 ...
 */
export async function initSession(creator, rounds, seconds) {
...
export async function updateUserResetVote(sessionId, roundNumber, username) {
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return update(child(roundRef, `votes/${username}`), {
    expire: 0,
    vote: "",
...
export async function updateUserEditVote(sessionId, roundNumber, username) {
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return update(child(roundRef, `votes/${username}`), {
    expire: 0,
    vote: "",
