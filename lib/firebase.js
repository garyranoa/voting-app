...
import {
  saveMajorityVotes,
  getRandomMajorityVotes,
  getQuestionMajorVotes,
} from "./firebaseResults"
...
export async function initSession(creator, rounds, seconds, options) {
  ...
  const initialState = {
    ...
    createdOn: serverTimestamp(), //new Date().toLocaleString(),
    roundNumber: 1,
    nextSessionId: "",
    _qIndex: 0,
  }
...
export async function newGameSession(baseSessionId) {
  ...
  if (mvq.length > 0) {
    await saveMajorityVotes(baseSessionId, mvq)
  }
...
export async function getOrderedQuestion(sessionId) {
  ...
  const result =
    sessionContent.roundNumber == 1
      ? await getQuestion(sessionContent._qIndex, sessionId)
      : await getQuestionMajorVotes(
          sessionContent._qIndex,
          sessionContent.baseSessionId,
          sessionId
        )
  if (!result.exceed) {
  ...
  const alreadyVoted = await checkQuestionAlreadyVoted(
    sessionId,
    result.question.id
  )
...
export async function updateRoundState(
  ...
  const sessionData = await getSessionValue(sessionId)
  const endAt = moment(serverTimestamp())
  ...
  if (state === ROUND_STATES.RESULTS) {
    // REMOVED: 01/09/2024 - calculation will now be at useRoundStats.js
  ...
  const sessionRef = child(sessions, sessionId)
  update(sessionRef, { timer })
  return update(roundRef, { state, endAt, finalVote }).catch((error) => error)
}
