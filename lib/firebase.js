 ...
 * @return {Promise<void>}
 */
export async function newRound(sessionId) {  const sessionContent = await getSessionValue(sessionId);
  ...
  const voters = sessionContent.voters
    ...
    : [];
  const options = await getRandomFeature();
...
export async function updateWord(sessionId, roundNumber) {
  const options = await getRandomFeature();
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`);
  return update(roundRef, { options: options, seed: generateRandomSeed() }).catch(
    (error) => error
  );
}
