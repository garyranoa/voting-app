...
export async function newGameSession(baseSessionId) {
  ...
  await update(prevSessionRef, { nextSessionId: sessionId, state: GAME_STATES.NEXTROUND }).catch(
    (error) => error
  )
...
export async function newRound(sessionId, useKeepQuestions = false) {
  ...
  const question = useKeepQuestions
    ? await getRandomMajorityVotes()
    : await getQuestion(sessionContent._qIndex)
...
export async function updateWord(sessionId, qIndex, roundNumber, delta = 1) {
  const sessionRef = child(sessions, sessionId)
  const question = await getQuestion(qIndex); //await getRandomQuestion()
  const roundRef = child(sessions, `/${sessionId}/rounds/${roundNumber}`)
  return update(roundRef, {
    question: question,
    seed: generateRandomSeed(),
  }).then(() => 
      update(sessionRef, { _qIndex: increment(delta) })
    )
  .catch((error) => error)

  //if ( sessionId && !checkQuestionAlreadyVoted(sessionId, selected.id)) { getQuestion() }
}
...
export async function updateUserVote(sessionId, roundNumber, username, vote) {
  ...
  return update(child(roundRef, `votes/${username}`), {
    ...
    .then((r) => {
      ...
      updateRoundState(sessionId, roundNumber, roundData.state)
    })
    .catch((error) => error)
}
